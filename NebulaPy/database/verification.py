...
class _NoProgress:
    """Preserve verification control flow without a progress-bar dependency."""
...
def authoritative_file_metadata(relative):
    """Return fixed metadata for one immutable database file."""
    entry = _load_authoritative_manifest()["stable_files"].get(relative)
    if not isinstance(entry, dict):
        raise InstallerError(
            f"authoritative manifest has no immutable file {relative}"
        )
    return entry
...
def verify_installation(database, args):
    ...
    print(f"  Component verification complete: {expected} files")
    return expected
