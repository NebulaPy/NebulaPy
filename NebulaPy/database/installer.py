...
_POWR_SED_SLOTS = threading.BoundedSemaphore(4)
_POWR_SERIAL_MODE = threading.Event()
_POWR_SERIAL_ENTRY_LOCK = threading.Lock()
...
@contextmanager
def _powr_sed_slot():
    """Use four request slots normally and all slots for recovery retries."""
    permits = 4 if _POWR_SERIAL_MODE.is_set() else 1
    if permits == 4:
        with _POWR_SERIAL_ENTRY_LOCK:
            for _ in range(permits):
                _POWR_SED_SLOTS.acquire()
    else:
        _POWR_SED_SLOTS.acquire()
    try:
        yield
        ...
        for _ in range(permits):
            _POWR_SED_SLOTS.release()


class _NoProgress:
    """Preserve download control flow without a progress-bar dependency."""
...
def _download_status(name, current, expected, *, status="Downloading", final=False):
    count = str(current) if final and current >= expected else f"{current}/{expected}"
    line = f"  {name:<24} files: {count:>7}  [{status}]"
    print(
        f"\r{line.ljust(80)}",
        end="\n" if final else "",
        flush=True,
...
def download_core(database, args):
    ...
    for component in ZENODO_COMPONENTS:
        ...
        if args.dry_run:
            ...
            continue
        status = _dataset_status(current, expected, force=args.force)
        _download_status(component["label"], 0, expected, status=status)
        ...
        _download_status(
            component["label"], expected, expected, status="Up to date", final=True
        )
...
def download_atlas(database, args):
    ...
    for grid in args.atlas_grid:
        ...
        if args.dry_run:
            ...
            continue
        status = _dataset_status(current, expected, force=args.force)
        _download_status(grid, 0, expected, status=status)
        ...
        with _progress() as progress, ThreadPoolExecutor(
            ...
            for future in as_completed(futures):
                ...
                completed += 1
                _download_status(grid, completed, expected, status=status)
        ...
        _download_status(grid, expected, expected, status="Up to date", final=True)
    if not args.dry_run:
...
def _validate_authoritative_sed(payload, relative, source):
    try:
        content = payload.decode("utf-8")
    except UnicodeDecodeError as error:
        raise InstallerError(f"{source}: SED is not UTF-8 ASCII text") from error
    _validate_sed(content, source)
    expected = authoritative_file_metadata(relative)
    if len(payload) != expected["size"]:
        raise InstallerError(
            f"{source}: incomplete SED size {len(payload)}; "
            f"expected {expected['size']}"
        )
    digest = hashlib.sha256(payload).hexdigest()
    if digest.lower() != expected["sha256"].lower():
        raise InstallerError(f"{source}: authoritative checksum mismatch")
    return content
...
def _download_powr_grid(database, official, local, display_name, args):
    ...
    if not args.force and parameter_table.is_file():
        try:
            ...
            for model in models:
                ...
                _validate_authoritative_sed(
                    sed_path.read_bytes(),
                    f"sed/powr/{local}-sed/{relative}",
    ...
    if not args.force:
        for model in models:
            ...
            try:
                _validate_authoritative_sed(
                    sed_path.read_bytes(),
                    f"sed/powr/{local}-sed/{sed_path.name}",
    ...
    if args.dry_run:
        ...
        return
    status = _dataset_status(current, expected, force=args.force)
    _download_status(display_name, 0, expected, status=status)
    target.mkdir(parents=True, exist_ok=True)
    records = {}
    hashes = {}
    ...
    def get_sed(worker_session, model):
        last_error = None
        relative = (
            f"sed/powr/{local}-sed/{local}_{model}_sed.txt"
        )
        for attempt in range(args.retries + 1):
            with _powr_sed_slot():
                sed = _powr_request(
            ...
            try:
                _validate_authoritative_sed(
                    sed.content, relative, f"{official}/{model}"
                )
                _POWR_SERIAL_MODE.clear()
                return sed.content
            except InstallerError as error:
                last_error = error
                _POWR_SERIAL_MODE.set()
                if attempt < args.retries:
                    time.sleep(2**attempt)
        raise last_error
    ...
    def fetch_powr(model):
        ...
        if not existing_is_valid:
            _write_bytes_atomic(destination, get_sed(worker_session, model))
        digest = hashlib.sha256(destination.read_bytes()).hexdigest()
        if args.delay:
            time.sleep(args.delay)
    ...
    with _powr_progress() as progress, ThreadPoolExecutor(
        ...
        for future in as_completed(futures):
            ...
            progress.advance(task)
            completed += 1
            _download_status(display_name, completed, expected, status=status)
    ...
    _download_status(
        display_name, expected, expected, status="Up to date", final=True
    )
...
def main(argv=None):
    ...
    try:
        ...
        if not args.dry_run:
            ...
            print(
                f"\n  NebulaPy Database {DATABASE_VERSION} verification PASSED"
            )
        print("\nDry run complete." if args.dry_run else "\nDownload complete.")
        print(f"Set NEBULAPY_DB={database}")
        return 0
    except (InstallerError, OSError) as error:
        print(f"nebulapy database install: error: {error}", file=sys.stderr)
        return 1
